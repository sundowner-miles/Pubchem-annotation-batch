...
def process_annotations(file_path,
    ...
    cid_col = find_col(cid_name, keywords=['cid', 'pubchem'])
    smiles_col = find_col(smiles_name, keywords=['smiles', 'csmiles', 'smile'])
    if cid_col is None or smiles_col is None:
        raise KeyError(f"找不到列。期望: '{cid_name}' 或 '{smiles_name}'。可用列: {df.columns.tolist()}")
    ...
    # 读取已处理 CID，跳过已完成部分（根据输出文件中的 CID 列）
    processed = set()
    header_needed = True
    ...
    pbar = tqdm(indices[start_idx:], total=total_to_process, desc="Processing CIDs")
    ...
    for i in pbar:
        cid = cid_list[i]
        if str(cid) in processed:
            continue
        smiles = smiles_list[i]

        name, description = fetch_annotation_by_cid(cid, verbose=verbose)
